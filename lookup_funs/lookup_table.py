...
for TSMC 180nm (and generic) gm/ID lookup tables (MAT v7.3 / HDF5).
"""
...
class LookupTable:
    ...
    def __init__(self, filepath):
        ...
        # Ensure VSB is ascending (config uses np.flip). Reorder dim 0 of all 4-D arrays.
        if hasattr(self, "VSB") and len(self.VSB) > 1 and self.VSB[0] > self.VSB[-1]:
            sort_idx = np.argsort(self.VSB)
            self.VSB = self.VSB[sort_idx]
            for key, arr in list(self.__dict__.items()):
                if isinstance(arr, np.ndarray) and arr.ndim == 4:
                    self.__dict__[key] = arr[sort_idx, :, :, :]
...
def lookup(data, outvar, *args, **kwargs):
    ...
    1. Basic lookup ::
        ...
        id = lookup(data, 'ID', 'VGS', 0.9, 'VDS', 0.9, 'L', 0.18, 'VSB', 0)

    2. Ratio lookup ::

        gm_id = lookup(data, 'GM_ID', 'VGS', 0.9, 'VDS', 0.9, 'L', 0.18)

    3. Cross-lookup (one ratio versus another) ::

        wt = lookup(data, 'GM_CGG', 'GM_ID', np.array([5, 10, 15]),
                    'VDS', 0.9, 'L', 0.18, 'VSB', 0)
...
def lookupVGS(data, *args, **kwargs):
    ...
    Mode 1 – known source terminal ::
        ...
        VGS = lookupVGS(data, 'GM_ID', 10, 'VDS', 0.9, 'VSB', 0.1, 'L', 0.1)
