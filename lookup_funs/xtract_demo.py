...
The script assumes that a TSMC 180 nm lookup table is available at the
path used in ``lookup_table.py`` / ``plot_demo.py``.
"""
...
# ------------------------------------------------------------------
# Example from Section A.1.4  (L = 60 nm, VDS = 0.6 V, VSB = 0 V)
# ------------------------------------------------------------------
# ------------------------------------------------------------------
# IMPORTANT: pick an L that actually exists in your lookup table.
# For the TSMC 180 nm library used in plot_demo.py, the shortest
# available channel is L = 0.18 µm.  If you have a more advanced
# node (e.g. 28 nm) you can use L = 0.06 or similar.
# ------------------------------------------------------------------
L = 0.18          # µm  (use 0.06 only if your LUT supports it)
VDS = 0.6         # V
VSB = 0.0         # V
rho = 0.6
TEMP = 300.0
...
print("=" * 50)
print("XTRACT output (L = 60 nm, VDS = 0.6 V)")
print("=" * 50)
