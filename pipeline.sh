#!/bin/bash
# =============================================================================
# L-level pipeline: simulate -> extract -> delete for TSMC 1.8V devices
...
# ----- Config ----------------------------------------------------------------
# RAM disk for temporary raw data (auto-created if not mounted)
RAMDIR="/mnt/ramraw"

# FINAL output directory for .mat files
# Default: VM shared folder -> Windows D:\IC617_share\tsmc18_lookup
MATDIR="/mnt/data/tsmc18_out"
...
setup_ramdisk() {
    if ! mountpoint -q "$RAMDIR"; then
        echo "[*] Creating tmpfs at $RAMDIR (size=13G) ..."
        mkdir -p "$RAMDIR"
        mount -t tmpfs -o size=13G tmpfs "$RAMDIR"
    fi
}

# ----- Helper: run one corner simulation -------------------------------------
# Output goes to terminal (VSB-level progress visible) + tee to log
run_sim() {
    local corner=$1 l_start=$2 l_end=$3
    local logfile="/tmp/sim_${corner}_L${l_start}.log"
    cd "$SCRIPTDIR"
    $PYTHON run_sim.py "$corner" $FINE --outdir "$RAMDIR" --L-range "$l_start" "$l_end" 2>&1 | tee "$logfile"
}

# ----- Helper: run one corner extraction -------------------------------------
run_extract() {
    local corner=$1 l_start=$2 l_end=$3
    local logfile="/tmp/ext_${corner}_L${l_start}.log"
    cd "$SCRIPTDIR"
    $PYTHON extract_new.py --voltage 18 $FINE \
...
clean_l() {
    ...
    for d in "$RAMDIR"/raw_tsmc18_*/; do
        if [ -d "$d" ]; then
...
check_failure() {
    ...
    for corner in $CORNERS; do
        if [ -f "/tmp/${stage}_${corner}_L${l_idx}.fail" ]; then
            failed=1
            echo "    [ERROR] $stage failed for corner=$corner L=$l_idx"
            echo "    Log: /tmp/${stage}_${corner}_L${l_idx}.log"
        fi
    done
    if [ $failed -ne 0 ]; then
...
# ----- Main ------------------------------------------------------------------
echo "========================================"
echo "Pipeline: 5 corners x 51 L, fine mode"
echo "RAM:      $RAMDIR"
echo "MAT:      $MATDIR"
echo "========================================"
...
cd "$SCRIPTDIR"
nL=$($PYTHON -c "from config_tsmc18 import get_config; c=get_config('tt', coarse=False); print(len(c['LENGTH']))")
    ...
    for corner in $CORNERS; do
        (
            if run_sim "$corner" "$l" "$l_next"; then
                rm -f "/tmp/sim_${corner}_L${l}.fail"
            else
                touch "/tmp/sim_${corner}_L${l}.fail"
            fi
        ) &
        pids+=($!)
    done
    ...
    for corner in $CORNERS; do
        (
            if run_extract "$corner" "$l" "$l_next"; then
                rm -f "/tmp/ext_${corner}_L${l}.fail"
            else
                touch "/tmp/ext_${corner}_L${l}.fail"
            fi
        ) &
        pids+=($!)
    done
